...
def build_aggregator_linear_allowlist(model):
    targets = set()
    for name, mod in model.named_modules():
        if not name.startswith("aggregator."):
            continue
        # Only inside frame_blocks or global_blocks
        if ".frame_blocks." not in name and ".global_blocks." not in name:
            continue
        # Pick exactly the Linear leaves we care about
        if name.endswith(".attn.qkv") or name.endswith(".attn.proj") or name.endswith(".mlp.fc1") or name.endswith(".mlp.fc2"):
            # Verify it's Linear to be safe
            if isinstance(mod, torch.nn.Linear):
                targets.add(name)
    return targets

def add_actquant(module,
                 name='',
                 ...
                 allow_names=None):
    """
    Wrap only modules whose fully qualified name is in allow_names.
    """
    # ipdb.set_trace()
    # allow_names = build_aggregator_linear_allowlist(module)
    if allow_names is None:
        raise ValueError("allow_names must be provided to restrict wrapping to aggregator.")

    if isinstance(module, ActQuantWrapper):
        return

    # Iterate attributes directly stored on this module
    for attr in dir(module):
        # ipdb.set_trace()
        tmp = getattr(module, attr)
        qualified = f"{name}.{attr}" if name else attr

        # Direct Linear attribute
        if type(tmp) in layers and not isinstance(tmp, ActQuantWrapper):
            if qualified in allow_names:
                setattr(module, attr, ActQuantWrapper(tmp))

        # Sequential children (index-qualified names)
        elif isinstance(tmp, torch.nn.Sequential):
            replaced = []
            for i, child in enumerate(tmp.children()):
                child_name = f"{qualified}.{i}"
                if type(child) in layers and not isinstance(child, ActQuantWrapper) and child_name in allow_names:
                    replaced.append(ActQuantWrapper(child))
                else:
                    replaced.append(child)
            setattr(module, attr, torch.nn.Sequential(*replaced))

        # ModuleList children (index-qualified names)
        elif isinstance(tmp, torch.nn.ModuleList):
            replaced = []
            for i, child in enumerate(tmp.children()):
                child_name = f"{qualified}.{i}"
                if type(child) in layers and not isinstance(child, ActQuantWrapper) and child_name in allow_names:
...
def find_qlayers(model, allow_names):
    qlayers = {}
    for n, m in model.named_modules():
        if n in allow_names and isinstance(m, ActQuantWrapper):
            qlayers[n] = m
    return qlayers
